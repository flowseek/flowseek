...
#define SESSION_PATH                    "/tmp/flow_session"
...
// 添加全局信号量指针
static sem_t *db_sem = NULL;
#define SEM_NAME "/ndpi_db_sem"

// 添加信号量初始化函数
static int init_semaphore() {
    // 创建或打开信号量
    db_sem = sem_open(SEM_NAME, O_CREAT, 0644, 1);
    if(db_sem == SEM_FAILED) {
        LOG(NDPI_LOG_ERROR, "Failed to create semaphore: %s\n", strerror(errno));
        return -1;
    }
    return 0;
}
...
static int init_database() {
    ...
    // 确保信号量已初始化
    if(!db_sem && init_semaphore() != 0) {
        return -1;
    }
    
    rc = sqlite3_open("/tmp/packet_info.db", &db);
    if(rc) {
        LOG(NDPI_LOG_ERROR, "无法打开数据库: %s\n", sqlite3_errmsg(db));
        return rc;
    }
    
    // 设置数据库文件权限为 666 (所有用户可读写)
    if(chmod("/tmp/packet_info.db", 0666) != 0) {
        LOG(NDPI_LOG_ERROR, "Failed to set database file permissions: %s\n", strerror(errno));
        // 不返回错误,因为这不是致命错误
    }
    ...
    if(rc != SQLITE_OK) {
        LOG(NDPI_LOG_ERROR, "SQL error: %s\n", err_msg);
...
void ndpi_workflow_free(struct ndpi_workflow * workflow) {
    ...
    // Free pcap_file if allocated
    if(workflow->prefs.pcap_file != NULL) {
        free((void *)workflow->prefs.pcap_file);
        workflow->prefs.pcap_file = NULL;
    }
  ...
  *proto = iph->protocol;
  ...
  if(ret == NULL) {
	...
	return(NULL);
      ...
      newflow->app_data_collected = 0; // 初始化标志位
...
*/
static void print_hex_data(u_int8_t *data, u_int16_t len) {
  u_int16_t i;
  for(i = 0; i < len; i++) {
    printf("%02X", data[i]);
    if((i+1) % 32 == 0) printf("\n");
...
void process_ndpi_collected_info(struct ndpi_workflow * workflow, struct ndpi_flow_info *flow) {
  ...
  if (workflow->ndpi_serialization_format != ndpi_serialization_format_unknown) {
    ...
    if(flow->detection_completed && (!flow->check_extra_packets)) {
      /* Print remaining application data if any */
      if(flow->app_data_len > 0) {
        printf("Flow ID: %u Application Data (%u bytes, padded to 784):\n", 
               flow->flow_id, flow->app_data_len);
        memset(flow->app_data + flow->app_data_len, 0, 784 - flow->app_data_len);
        print_hex_data(flow->app_data, 784);
      }
...
// 添加实时打印函数
static void print_packet_info(struct ndpi_flow_info *flow, struct ndpi_workflow *workflow) {
    if(!flow || !workflow) return;
    
    // 获取协议名称
    char buf[64], category[64];
    ndpi_protocol2name(workflow->ndpi_struct, flow->detected_protocol, buf, sizeof(buf));
    
    // 只处理已识别的协议
    if(strcmp(buf, "Unknown") != 0) {
        // 提前准备好所有数据,避免在持有信号量时处理
        char risks[512] = {0};
        if(flow->risk) {
            int offset = 0;
            for(int i = 0; i < NDPI_MAX_RISK; i++) {
                if(NDPI_ISSET_BIT(flow->risk, i)) {
                    offset += snprintf(risks + offset, sizeof(risks) - offset, 
                                     "%s%s", (offset ? "," : ""), ndpi_risk2str(i));
                }
            }
        }
        ...
        snprintf(category, sizeof(category), "%s", 
                ndpi_category_get_name(workflow->ndpi_struct,
                ndpi_get_proto_category(workflow->ndpi_struct, flow->detected_protocol)));
        ...
        if(sem_wait(db_sem) < 0) {
            LOG(NDPI_LOG_ERROR, "Failed to acquire semaphore: %s\n", strerror(errno));
            return;
        }
        ...
        const char *sql = "REPLACE INTO packet_info (flow_id, src_ip, src_port, dst_ip, dst_port, "
                         "protocol, category, hostname, up_packets, up_bytes, down_packets, down_bytes, "
                         "first_seen, last_seen, risks) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                         
        int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, 0);
        if(rc != SQLITE_OK) {
            LOG(NDPI_LOG_ERROR, "Failed to prepare statement: %s\n", sqlite3_errmsg(db));
            sem_post(db_sem);
            return;
        }
        
        // 绑定参数
        sqlite3_bind_int64(stmt, 1, flow->flow_id);
        sqlite3_bind_text(stmt, 2, flow->src_name, -1, SQLITE_STATIC);
        sqlite3_bind_int(stmt, 3, ntohs(flow->src_port));
        sqlite3_bind_text(stmt, 4, flow->dst_name, -1, SQLITE_STATIC);
        sqlite3_bind_int(stmt, 5, ntohs(flow->dst_port));
        sqlite3_bind_text(stmt, 6, buf, -1, SQLITE_STATIC);
        sqlite3_bind_text(stmt, 7, category, -1, SQLITE_STATIC);
        sqlite3_bind_text(stmt, 8, flow->host_server_name[0] ? flow->host_server_name : "", -1, SQLITE_STATIC);
        sqlite3_bind_int64(stmt, 9, flow->src2dst_packets);
        sqlite3_bind_int64(stmt, 10, flow->src2dst_bytes);
        sqlite3_bind_int64(stmt, 11, flow->dst2src_packets);
        sqlite3_bind_int64(stmt, 12, flow->dst2src_bytes);
        sqlite3_bind_int64(stmt, 13, flow->first_seen_ms);
        sqlite3_bind_int64(stmt, 14, flow->last_seen_ms);
        sqlite3_bind_text(stmt, 15, risks, -1, SQLITE_STATIC);
        ...
        // 如果发生错误,记录日志
        if(rc != SQLITE_DONE) {
            LOG(NDPI_LOG_ERROR, "Failed to insert data: %s\n", sqlite3_errmsg(db));
        }
        
        // 打印信息到控制台 (不需要在信号量保护范围内)
        printf("\n=== 数据包信息 [Flow ID: %u] ===\n", flow->flow_id);
        printf("地址: %s:%u <-> %s:%u\n",
               flow->src_name, 
               ntohs(flow->src_port),
        ...
        printf("流量统计:\n");
        printf("- 上行: %llu 包 / %llu 字节\n",
               (unsigned long long)flow->src2dst_packets,
               (unsigned long long)flow->src2dst_bytes);
        printf("- 下行: %llu 包 / %llu 字节\n",
               (unsigned long long)flow->dst2src_packets,
               (unsigned long long)flow->dst2src_bytes);
        
        printf("时间信息:\n");
        
        static char time_str[64];  // 静态缓冲区避免重复分配
        ...
        format_time_str(time_str, sizeof(time_str), flow->first_seen_ms);
        printf("- 首次见到: %s\n", time_str);
        
        // 打印最后见到时间
        format_time_str(time_str, sizeof(time_str), flow->last_seen_ms);
        printf("- 最后见到: %s\n", time_str);
...
static struct ndpi_proto packet_processing(struct ndpi_workflow * workflow,
  ...
  if(flow != NULL) {
    ...
    /* Collect application layer data */
    if(!flow->app_data_collected && flow->app_data_len < 784 && payload_len > 0) {
      u_int16_t copy_len = ndpi_min(payload_len, 784 - flow->app_data_len);
      memcpy(flow->app_data + flow->app_data_len, payload, copy_len);
      flow->app_data_len += copy_len;
      
      /* If we have collected 784 bytes, print the data */
    if(flow->app_data_len >= 784) {
        flow->app_data_collected = 1; // 设置标志位表示已经收集完成
        // 确保目录存在
        ...
        // 获取pcap文件名（去掉路径和扩展名）
        if (workflow->prefs.pcap_file == NULL) {
            LOG(NDPI_LOG_ERROR, "pcap_file is NULL\n");
            printf("pcap_file is NULL\n");
            return(nproto);
        }

        const char *pcap_file = strrchr(workflow->prefs.pcap_file, '/');
        pcap_file = pcap_file ? pcap_file + 1 : workflow->prefs.pcap_file;

        char pcap_name[128];
        // 使用 snprintf 更安全地复制字符串
        snprintf(pcap_name, sizeof(pcap_name), "%s", pcap_file);

        // 查找并去掉扩展名
        char *dot = strrchr(pcap_name, '.');
        if (dot) {
            *dot = '\0';  // 去掉扩展名
        }

        if(strncmp(pcap_name, "unknown", strlen("unknown")) == 0)
            snprintf(pcap_name, sizeof(pcap_name), "flow_%d", flow->flow_id);
        
        
        // 生成符合要求的文件名
        snprintf(png_filename, sizeof(png_filename), "%s/%s.%s_%s_%u_%s_%u.png",
                SESSION_PATH,
                pcap_name,
        ...
        // Print hex data before saving
        printf("Flow ID: %u Application Data (784 bytes):\n", flow->flow_id);
        printf("Flow png: %s\n", png_filename);
        for(int i = 0; i < 784; i++) {
            printf("%02X ", flow->app_data[i]);
            if((i+1) % 16 == 0) printf("\n");  // New line every 16 bytes
        }
        printf("\n");
        
        if(save_as_png(png_filename, flow->app_data) == 0) {
            printf("Flow ID: %u Application Data saved as PNG: %s\n", flow->flow_id, png_filename);
        } else {
            printf("Flow ID: %u Failed to save Application Data as PNG\n", flow->flow_id);
        }
        
        memset(flow->app_data, 0, 784);
        flow->app_data_len = 0;
    }
    }
    ...
    print_packet_info(flow, workflow);
